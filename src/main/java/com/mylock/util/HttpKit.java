...
public class HttpKit {
	...
	private static void readResponseString(HttpURLConnection conn, OutputStream out) {
		try {
			InputStream in = conn.getInputStream();
			byte[] cbuf=new byte[1024 * 10];
			int len = 0;
	 ...
	 * @param out
	 */
	public static void getDownload(String url, Map<String, String> queryParas, Map<String, String> headers, OutputStream out) {

		HttpURLConnection conn = null;
		try {
			conn = getHttpConnection(buildUrlWithQueryString(url, queryParas), GET, headers);
			conn.connect();
			readResponseString(conn, out);
		}
		catch (Exception e) {
			throw new RuntimeException(e);
		}
	...
	public static void getDownloadRetryTimes(String url, Map<String, String> queryParas, Map<String, String> headers, OutputStream out, int times) {

		if (times < 0) throw new IllegalArgumentException("The number of retries cannot be less than 0");

		for (int i = 0; i < 1 + times; i++) {
			try {
				getDownload(url, queryParas, headers, out);
				break;
			} catch (Exception e) {
				log.error("第"+(i+1)+"次下载失败, url:" + url, e);
				try {
